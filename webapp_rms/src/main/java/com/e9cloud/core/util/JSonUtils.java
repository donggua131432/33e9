...
public class JSonUtils {
     ...
     * 比如List<Student>，就不能直接进行转换了。 因为readValue(json
    ...
    public static <T> T readValue(String content, Class<? extends Collection> arrayType, Class<?> ofType) {
        if (objectMapper == null) {
            objectMapper = new ObjectMapper();
        }
        try {
            return objectMapper.readValue(content, getCollectionType(arrayType, ofType));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static JavaType getCollectionType(Class<? extends Collection> collectionClass, Class<?> elementClasses) {
        ...
        return objectMapper.getTypeFactory().constructCollectionType(collectionClass, elementClasses);
    }

    /**
     * 把JavaBean转换为json字符串 (1)普通对象转换：toJson(Student) (2)List转换：toJson(List)
     * (3)Map转换:toJson(Map) 我们发现不管什么类型，都可 以直接传入这个方法
     *
     * @param object
     *            JavaBean对象
     * @return json字符串
