...
public class POIXlsView extends AbstractXlsView {
    ...
    private static int DEFAULT_WIDTH_17 = 20 * 256;
    private static int DEFAULT_WIDTH_60 = 60 * 256;
    ...
    private void addColumNameToWsheet(HSSFSheet hssfSheet, HSSFWorkbook wb) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            setcwidth(hssfSheet, Tools.toStr(text), i, 3);
        }
    }
    ...
    private void writeContext(HSSFSheet hssfSheet, List<Map<String, Object>> list, HSSFWorkbook wb) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = 0, len = columnEnames.size(); j < len; j++) {
                ...
                setcwidth(hssfSheet, valueCell, j, 2);
            }
        }
    }

    public void setcwidth(HSSFSheet hssfSheet, String valueCell, int index, int rate){
        int length = Tools.toStr(valueCell).length();
        int width = DEFAULT_WIDTH;
        if (length > 3 && length < 30) {
            width = DEFAULT_WIDTH * length * rate;
        }
        if (length >= 30) {
            width = DEFAULT_WIDTH_60;
        }

        width = (width > hssfSheet.getColumnWidth(index) ? width : hssfSheet.getColumnWidth(index));
        hssfSheet.setColumnWidth(index, width);
    }
