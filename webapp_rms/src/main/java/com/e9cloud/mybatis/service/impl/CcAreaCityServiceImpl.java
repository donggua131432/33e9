...
public class CcAreaCityServiceImpl extends BaseServiceImpl implements CcAreaCityService {
     ...
     */
    @Override
    public List<Tree> addArea(CcArea ccArea, String[] codes)throws BadHanyuPinyinOutputFormatCombination {
        List<Tree> citys = filteAreaCity(ccArea, codes);

        if (citys == null || citys.size() == 0) {
            String eareId = ID.randomUUID();
            ccArea.setAreaId(eareId);
            ccArea.setPinyin(PinyinUtils.toPinYin(ccArea.getAname()));
            ccArea.setStatus("00");
     ...
     */
    @Override
    public List<Tree> editArea(CcArea ccArea, String[] codes)throws BadHanyuPinyinOutputFormatCombination {
        List<Tree> citys = filteAreaCity(ccArea, codes);
