...
var _getPostParams;
//初始化 swf
function initSwfupload(url,sizeLimit,types,typsdesc,uploadLimit,getPostParams){
		_getPostParams = getPostParams;
		//初始化界面
		...
		var settings={
			...
			flash_url : contextPath + "/js/plugins/swfupload/swfupload.swf",
			...
			// Button Settings
			//button_image_url : contextPath + "/plugins/swfupload/select-files-zh_CN.png",
...
function init(uploadLimit,sizeLimit){
	...
	var uploadbody = '<div id="uploadBody" class="ke-dialog-upload">'
		...
		+'	<span class="ke-button-common ke-button-outer" title="取消"><input id="btncancel" class="ke-button-common ke-button" type="button" value="取消"></span>'
		+'  </div>'
		+'	</div>'
		+'';
...
function swfUploadLoaded() {
	...
	$("#"+this.customSettings.btnUpload_ID).click(function (){
		var postParm = _getPostParams();
		if(!postParm){
			alert("请选择小节！");
			return
		}
		setPostParams(postParm);
		swfu.startUpload()
	});
}
...
function uploadProgress(file, bytesLoaded) {
	try {
		...
		if (percent === 100) {
			progress.setStatus("上传成功");//正在创建缩略图...
...
function addReadyFileInfo(fileid,fileName,message){
	var photoTr = '<tr id="'+fileid+'">\
					<td><div class="ke-name" alt="'+fileName+'">'+fileName+'</div></td>\
					<td>\
						<div class="ke-progressbar">\
					        <div class="ke-progressbar-bar" style="width: 150px;height: 10px;background-color: rgb(186, 172, 154);">\
		                        <div class="ke-progressbar-bar-inner" style="background-color: green; width : 0%;height: 10px;"></div>\
		              		</div>\
		            		<div class="ke-progressbar-percent">0%</div>\
		            	</div>\
		            </td>\
					<td><div class="ke-message">'+message+'</div></td>\
					<td></td>\
					<td><a class="btn btn-small btn-danger ke-delete" title="删除"><span class="green"><i class="icon-trash"></i></span></a></td>\
				</tr>';
	...
	$("#"+swfu.customSettings.photoContainer_Id).append(photoTr);
...
function setPostParams(paramsObject){
	swfu.setPostParams(paramsObject);
}
