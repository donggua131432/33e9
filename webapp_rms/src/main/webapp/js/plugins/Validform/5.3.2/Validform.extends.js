...
// url
$.Datatype.url = function(gets,obj,curform,regxp) {
    if (o._regex.url.test(gets)) {
        return true;
    }
    return "格式错误";
};
...
// 图片格式
    ...
    try{
        var path = gets.substring(gets.lastIndexOf(".")).substring(1).toLowerCase();
        if(!reg.test(path)){
            return "文件格式不正确";
        }
    }catch(e){
...
// 音频文件
$.Datatype.video = function(gets,element,curform,regxp){
    ...
    if(ele.files){ // element.files 是chrome，firefox等浏览器的对戏那个，如果是ie的话他的值就是false
        console.log("element.files");
        var thisFile = ele.files[0]; // 获取文件的对像
        fileSize = thisFile.size;//获取当前上传的文件的大小
    } else { // 如果是ie
        console.log("element.files else");
    }
