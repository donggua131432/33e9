...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            //dataTablesAjax 选项
            var dataTablesAjax = {
                url: '<c:url value='/axbRate/pageAxbRateUnion'/>',
                type: 'POST',
                "data": function(d){
            ...
            var columnDefs = [
                ...
                {"targets": [9], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        return $.calcEval("${axbRate.per60}*"+full.per60_discount+"/1000").toFixed(4);
                    }
                },
                ...
                {"targets": [11], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        return (full.rent_discount/10);
                    }
                },
                {"targets": [12], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        return $.calcEval("${axbRate.rent}*"+full.rent_discount+"/1000").toFixed(4);
                    }
                },
                {"targets": [13], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        return ${axbRate.recording};
                    }
                },
                {"targets": [14], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        if(full.recording_discount){
                           return  full.recording_discount
                        }
                        return 0;
                ...
                {"targets": [15], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        if(full.recording_discount){
                            return $.calcEval("${axbRate.recording}*"+full.recording_discount+"/1000").toFixed(4);
                        }
                        return 0;
                    }
                },
                {"targets": [16], "data": "id", "sClass" : "text-c",
                    "render": function(data, type, full) {
                        return '<a title="查看" href="javascript:;" onclick="restRate_openDialog(\'隐私小号费率配置详情\',\'<c:url value="/axbRate/getAxbRateDetail?feeid='+full.feeid+'"/>\',\'1200\',\'700\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont" style="font-size: 18px; color: green;">&#xe695;</i>查看</a>';
                    }
                }
            ];
        ...
        /*管理员-增加*/
        function restRate_openDialog(title,url,w,h){
...
<div class="pd-20">
    ...
    <%--<div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a href="javascript:;" onclick="restRate_openDialog('新增专线语音费率配置','<c:url value="/restRate/addRestRate"/>','800','720')" class="btn btn-primary radius">
