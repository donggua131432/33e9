...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            var dataTablesAjax = {
                url: '<c:url value='/eccShowNumPool/pageEccShowNumList'/>',
                type: 'POST',
                "data": function(d){
...
//                },
...

                {
                    ...
                    "render": function(data, type, full) {
                        var _ex_in = "";
                        if (full.status == '03') {
                            _ex_in += '<a title="编辑" href="javascript:;" onclick="openDialog(\'编辑\',\'<c:url value="/eccShowNumPool/toEdit?id='+full.id+'"/>\',\'450\',\'300\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i>编辑</a>';
                        }
                        return _ex_in;
                    }
                }
        ...
        function addPhone(){
            openDialog('外显号码添加','<c:url value="/eccShowNumPool/toAdd"/>','450','300');
        }

        function importPhone(){
            openDialog('批量导入', '<c:url value="/eccShowNumPool/import"/>', '600', '260');
        }
        ...
        function download() {
            ...
            window.open("<c:url value='/eccShowNumPool/download'/>?" + formData);
        }
            ...
            if (strId) {
                layer.confirm('是否确认删除？', {
                    ...
                    $.ajax({
                        ...
                        url: "<c:url value='/eccShowNumPool/deleteEccShowNums'/>",
...
<div class="page-container" style="padding: 20px;">
    <%--<form class="form form-horizontal" id="form-article-add">--%>
        <div id="tab-system" class="HuiTab">
            <div ><H2><span>外显号码</span></H2></div>
            <div class="tabCon">
                <form id="searchFrom" class="form form-horizontal">
                    <div class="row cl">
                        外显号码：
                        <input type="text" class="input-text" style="width:186px" placeholder="请输入外显号码" id="number" name="number"/>&nbsp;&nbsp;&nbsp;&nbsp;
                        状态：
