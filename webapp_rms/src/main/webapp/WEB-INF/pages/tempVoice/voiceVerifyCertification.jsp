    ...
    <form class="form form-horizontal" id="form-edit">
        <div class="row cl">
            ...
            <div class="formControls col-8" style="line-height: 2">
                <c:if test="${voiceVerify.type eq '00'}">语音</c:if>
                <c:if test="${voiceVerify.type eq '01'}">文本</c:if>
            </div>
        </div>
        ...
        <c:if test="${voiceVerify.type eq '01'}">
            <div class="row cl">
                <label class="form-label col-4">模板内容：</label>
                <div class="formControls col-8" style="line-height: 2">
                        ${voiceVerify.tContent}
                </div>
            </div>
        </c:if>

        <!--- 已审核过的 --->
        <c:if test="${voiceVerify.auditStatus ne '00'}">
            <div class="row cl">
                <label class="form-label col-4">审核：</label>
                <div class="formControls col-8" style="line-height: 2;">
                    <c:if test="${voiceVerify.auditStatus eq '01'}">审核通过</c:if>
                    <c:if test="${voiceVerify.auditStatus eq '02'}">审核不通过</c:if>
                </div>
            </div>
        </c:if>
        <c:if test="${voiceVerify.auditStatus ne '01'}">
            <div class="row cl">
                ...
                <div class="formControls col-8" style="line-height: 2;word-break: break-all; word-wrap: break-word;width: 300px;">
                    <c:out value="${voiceVerify.auditCommon}"/>
                </div>
            </div>
        </c:if>
        <c:if test="${voiceVerify.auditStatus eq '01' and voiceVerify.type eq '01'}">
            <div class="row cl">
                <label class="form-label col-4">生成状态：</label>
                <div class="formControls col-8" style="line-height: 2;word-break: break-all; word-wrap: break-word;width: 300px;">
                    <c:if test="${empty voiceVerify.vUrl}">
                        生成中
                    </c:if>
                    <c:if test="${not empty voiceVerify.vUrl}">
                        已完成
                    </c:if>
                </div>
            </div>
        </c:if>
        <c:if test="${voiceVerify.type eq '00'}">
            <div class="row cl">
                <label class="form-label col-8">
                    <audio id="voice" src="<c:url value="/voice/verify/${voiceVerify.vUrl}"/>" controls="controls">
                        您的浏览器不支持 audio 标签。
                    </audio>
                </label>
                <div class="formControls col-4" style="line-height: 2">
                    <a style="color: blue" href="<c:url value="/voiceVerify/download?url=${voiceVerify.vUrl}&vType=${voiceVerify.type}"/>" download="${voiceVerify.vUrl.split("/")[1]}">语音模板下载</a>
                </div>
            </div>
        </c:if>
        <c:if test="${voiceVerify.type eq '01'}">
        ...
        <!--- 待审核的 --->
        <c:if test="${voiceVerify.auditStatus eq '00'}">
            <div class="row cl">
                <label class="form-label col-4">审核：</label>
...
<script type="text/javascript">
    function submitToAuth() {
        ...
        $.ajax({
            type: "POST",
            url: "<c:url value="/voiceVerify/audit"/>",
            data: {id:id, auditStatus:auditStatus, auditCommon:auditCommon,type:'${voiceVerify.type}'},
