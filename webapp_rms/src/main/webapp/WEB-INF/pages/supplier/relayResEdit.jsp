...
<head>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            initPCArea("pcode", "cityid", "请选择", '${res.pcode}', '${res.cityid}');
            initSelect("relayNum", "<c:url value='/relay/getRelaysForRes'/>?useType=01&resId=${res.id}", "relayNum", "relayName", "请选择", '${res.relayNum}');
            ...
            $("#areaCode").text($("#cityid option:selected").attr("data-code"));
        });
...
<body>
<div class="pd-20">
    <form action="<c:url value="/relayRes/updateRes"/>" method="post" class="form form-horizontal" id="addSupplier">
        <input type="hidden" name="id" value="${res.id}" />
        <input type="hidden" name="oldRelayNum" value="${res.relayNum}"/>
        <div class="row cl">
            <label class="form-label col-2"><span class="c-red">*</span>资源名称：</label>
            <div class="formControls col-7">
                <input type="text" class="input-text" value="${res.resName}" placeholder="请输入资源名称" id="resName" name="resName" datatype="/^[\u4E00-\u9FA50-9a-zA-Z]{1,20}$/" maxlength="20" errormsg="资源名称不合法" nullmsg="资源名称不能为空"/>
            </div>
            <div class="col-3"></div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-7">
                中继ID：<span id="relayNums"><c:out value="${res.sipBasic.relayNum}"/></span>
            </div>
            <div class="col-2"></div>
        </div>
        ...
        <div class="row cl">
            <label class="form-label col-3"></label>
            <div class="formControls col-7">
                外域对端IP和端口：<span id="ipport1"><c:out value="${res.sipBasic.ipport1}"/></span>
            </div>
            <div class="col-2"></div>
        </div>
        <div class="row cl">
            <label class="form-label col-3"></label>
            <div class="formControls col-7">
                外域本端IP和端口：<span id="ipport2"><c:out value="${res.sipBasic.ipport2}"/></span>
            </div>
            <div class="col-2"></div>
        </div>
        <div class="row cl">
            <label class="form-label col-3"></label>
            <div class="formControls col-7">
                内域对端IP和端口：<span id="ipport3"><c:out value="${res.sipBasic.ipport3}"/></span>
            </div>
            <div class="col-2"></div>
        </div>
        <div class="row cl">
            <label class="form-label col-3"></label>
            <div class="formControls col-7">
                内域本端IP和端口：<span id="ipport4"><c:out value="${res.sipBasic.ipport4}"/></span>
            </div>
            <div class="col-2"></div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-7">
                <input type="text" class="input-text" value="${res.relayCnt}" placeholder="请输入线路数" id="relayCnt" name="relayCnt" datatype="empty|/^(0|[1-9][0-9]{0,7})$/" maxlength="10" errormsg="请输入整数"/>
            </div>
            <div class="col-3"></div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-7">
                <input type="text" class="input-text" value="${res.relayRent}" placeholder="总低消" id="relayRent" name="relayRent" datatype="empty|f7-4" maxlength="11" errormsg="请输入整数" />
            </div>
            <div class="col-3"></div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-7">
                <select class="input-text" id="operator" name="operator" datatype="*" nullmsg="请选择运营商">
                    ...
                    <option value="00" <c:if test="${res.operator eq '00'}">selected</c:if>>中国移动</option>
                    <option value="01" <c:if test="${res.operator eq '01'}">selected</c:if>>中国联通</option>
                    <option value="02" <c:if test="${res.operator eq '02'}">selected</c:if>>中国电信</option>
                    <option value="06" <c:if test="${res.operator eq '06'}">selected</c:if>>其他</option>
                </select>
            </div>
            <div class="col-3"></div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-7">
                <select class="input-text" name="signType" id="signType" datatype="*" nullmsg="请选择签署类型">
                    <option value="">请选择</option>
                    <option value="00" <c:if test="${res.signType eq '00'}">selected</c:if>>运营商直签</option>
                    <option value="01" <c:if test="${res.signType eq '01'}">selected</c:if>>第三方资源</option>
                </select>
            </div>
            <div class="col-3"></div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-7">
                <input class="input-text" style="width: 160px" name="per" datatype="f7-4" value="${res.per}" nullmsg="请输入价格"/>
                &nbsp;元&nbsp;/&nbsp;&nbsp;
                <select class="input-text" style="width: 60px" id="cycle" name="cycle">
                    <option value="6"  <c:if test="${res.cycle eq '6'}">selected</c:if>>6秒</option>
                    <option value="60" <c:if test="${res.cycle eq '60'}">selected</c:if>>分</option>
                </select>
            </div>
            <div class="col-3"></div>
        </div>
    ...
    <c:if test="${not empty res.relayResPers}">
        <c:forEach items="${res.relayResPers}" var="per">
            <div class="row cl" name="pertr">
                <label class="form-label col-1"></label>
                <div class="col-1">
                    <select class="input-text" style="width: 60px" name="persOperator">
                        <option value="00" <c:if test="${per.operator eq '00'}">selected</c:if>>移动</option>
                        <option value="01" <c:if test="${per.operator eq '01'}">selected</c:if>>联通</option>
                        <option value="02" <c:if test="${per.operator eq '02'}">selected</c:if>>电信</option>
                    </select>
                </div>
                <div class="col-1">
                    <select class="input-text" style="width: 60px" name="persCallType">
                        <option value="00" <c:if test="${per.callType eq '00'}">selected</c:if>>市话</option>
                        <option value="01" <c:if test="${per.callType eq '01'}">selected</c:if>>长途</option>
                    </select>
                </div>
                <div class="col-1">
                    <select class="input-text" style="width: 60px" name="persNumType">
                        <option value="00" <c:if test="${per.numType eq '00'}">selected</c:if>>手机</option>
                        <option value="01" <c:if test="${per.numType eq '01'}">selected</c:if>>固话</option>
                        <option value="04" <c:if test="${per.numType eq '04'}">selected</c:if>>其他</option>
                    </select>
                </div>
                <div class="col-2" name="unq"></div>
                <div class="col-2">
                    <input class="input-text" value="${per.per}" style="width: 80px" name="persPer" datatype="f7-4"/>
                    &nbsp;元&nbsp;&nbsp;&nbsp;/&nbsp;
                </div>
                <div class="col-1">
                    <select class="input-text" style="width: 60px" name="persCycle">
                        <option value="6"  <c:if test="${per.cycle eq '6'}">selected</c:if>>6秒</option>
                        <option value="60" <c:if test="${per.cycle eq '60'}">selected</c:if>>分</option>
                    </select>
                </div>
                <div class="col-1">
                    <input type="hidden" name="resPers" value=""/>
