            ...
            callback:function(data){
                ...
                "relayUnique" : function(gets,obj,curform,regxp){
                    ...
                    if(isUnique){
                        ...
                        var relayArray = ${relayStr};
                        var relayFlag = false;
                        $.each(relayArray,function(n,value) {
                                if(gets == value.relay){
                                    relayFlag = true;
                                }
                        });
        ...
        if ($("div[name='relayDiv']").length <= 4) {
            var itemHtml = '<div class="row cl" name="relayDiv">\
                ...
                <div class="formControls col-7">\
                    <select  name="relay"  style="width: 170px;height: 28px;" datatype="relayUnique">\
                    <c:forEach items="${relays1}" var="relay">\
                        <option value="${relay.relayNum}">${relay.relayName}</option>\
                    </c:forEach>\
                    </select>\
                    权重：\
                    <input type="text" value="1" style="width: 70px;height: 22px;"  name="weight" max="9" datatype="weight" maxlength="1"/>\
...
<body>
<div class="pd-20">
    <form action="<c:url value='/ccinfo/edit'/>" method="post" class="form form-horizontal" id="form-member-add">
        <input type="hidden" name="relays" id="relays" value=""/>
        <input type="hidden" name="weights" id="weights" value=""/>
        <input type="hidden" value="${ccInfo.id}" name="id"/>
        <input type="hidden" value="${ccInfo.subid}" name="subid"/>
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <select id="name" style="width: 306px" nullmsg="客户名称不能为空！" datatype="*">
                    <c:if test="${not empty ccInfo.name}">
                        <option value="${ccInfo.sid}">${ccInfo.name}</option>
                    </c:if>
                </select>
            </div>
            <div class="col-4"> </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text" value="${ccInfo.sid}" placeholder="请输入账户ID" id="sid" name="sid" datatype="*2-32" nullmsg="账户ID不能为空！" readonly/>
            </div>
            <div class="col-4"> </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <label class="form-label col-3" id="appName">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text" value="${ccInfo.appid}" placeholder="请输入应用ID" id="appid" name="appid" datatype="*2-32" nullmsg="应用ID不能为空！" readonly/>
            </div>
            <div class="col-4"> </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text" value="${ccInfo.ccname}" placeholder="请输入呼叫中心名称" id="ccname" name="ccname" datatype="*2-32&_ajax" data-ajax="{'url':'<c:url value="/ccinfo/checkCcnameUnique?appid=${ccInfo.appid}"/>',param:[{name:'id',value:${ccInfo.id}}],msg:'该呼叫中心名称已经存在'}" nullmsg="呼叫中心名称不能为空！"/>
            </div>
            <div class="col-4"> </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text" value="${ccInfo.ccoMaxNum}" placeholder="请输入最大话务员数量" id="ccoMaxNum" name="ccoMaxNum" ignore="ignore" datatype="n1-6"/>
            </div>
            <div class="col-4"> </div>
        </div>
                ...
                <div class="skin-minimal">
                    <div class="check-box">
                        <c:if test="${ccInfo.defaultCc == 1}">
                            <input type="radio" name="defaultCc" value="1"  checked="checked">
                        </c:if>
                        <c:if test="${ccInfo.defaultCc != 1}">
                    ...
                    <div class="check-box">
                        <c:if test="${ccInfo.defaultCc == 0}">
                            <input type="radio" name="defaultCc" value="0" checked="checked">
                        </c:if>
                        <c:if test="${ccInfo.defaultCc != 0}">
                            <input type="radio" name="defaultCc" value="0">
                        </c:if>
                        <label>否</label>
                    </div>
                </div>
        ...
        <div id="editRelaychDiv" >
            <c:forEach items="${relayWeigths}" var="relayWeigth" varStatus="status">
                <div class="row cl" name="relayDiv">
                    <label class="form-label col-3">中继群：</label>
                    <div class="formControls col-7">
                        <select  name="relay"  style="width: 170px;height: 28px;" datatype="relayUnique">
                            <option value="${relayWeigth.relay}" selected>${relayWeigth.name}</option>
                            <c:forEach items="${relays1}" var="relay">
                                <option value="${relay.relayNum}" >${relay.relayName}</option>
                            </c:forEach>
                        </select>
                        权重：
                        <input type="text" value="${relayWeigth.weight}" style="width: 70px;height: 22px;"  name="weight" max="9" datatype="weight" maxlength="1"/>
                        <button type="button" onclick="delRelayItem(this);" class="delBtn marLeft15">删除</button>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text" value="${ccInfo.lifeRelay}" placeholder="请输入逃生号码" id="lifeRelay" name="lifeRelay" ignore="ignore" datatype="/^[1][0-9]{10}$/|/^[0][0-9]{10,11}$/|/^[4][0][0][0-9]{1,20}$/"/>
            </div>
            <div class="col-4"> </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text" value="${ccInfo.dtmfNum}" placeholder="请输入DTMF收号" id="dtmfNum" name="dtmfNum" datatype="/^\s*$/|/^[0-9,|,\*]{3,255}$/"/>
            </div>
            <div class="col-4"> </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-7">
                <textarea id="remark" name="remark" rows="5" class="textarea"  maxlength="250" placeholder="请输填写备注信息" dragonfly="true" onKeyUp="textarealength(this,500)">${ccInfo.remark}</textarea>
