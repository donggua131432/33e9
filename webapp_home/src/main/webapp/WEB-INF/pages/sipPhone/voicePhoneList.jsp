...
<script id="integralTemplate" type="text/x-jquery-tmpl">
	<tr class="date_hg">
		...
		{{if status=='01'||status=='02'}}
				  <a href="<c:url value='/voicePhone/updatePage'/>?appid=\${appid}">编辑</a>
				  <a href="<c:url value='/voicePhone/view'/>?appid=\${appid}">查看</a>
		...
		{{/if}}
		{{if status==''}}
                    <a href="<c:url value='/voicePhone/savePage'/>?appid=\${appid}">新建</a>
		{{/if}}
		</td>
		<td>\${common}</td>
	</tr>
...
<script>
    ...
    function delete1(){
        $.post("<c:url value='/voicePhone/del'/>",{appid : appid},function(result){
            if(result.status == "00"){
    ...
    // 铃声列表
    $("#apptable").page({
        url:"<c:url value='/voicePhone/query'/>",
        pageSize:5,
