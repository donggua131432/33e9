...
<script type="text/javascript">
    ...
    function loadCharts(sType){
        var data = loadAjaxData('<c:url value="/monitor/maskMonitorData"/>',{sType:sType,day:$("#historyDateTime").val()}),
        ...
        thscsumSeries.push(initSeries('C路','#7D00FF','#E6D2FF',chartsData.thscsumC));
        ...
        succcntSeries.push(initSeries('C路','#7D00FF','#E6D2FF',chartsData.succcntC));
        ...
        answercntSeries.push(initSeries('C路','#7D00FF','#E6D2FF',chartsData.answercntC));

        $("#spanLineA").text(chartsData.callTimeLengthA);
        $("#spanLineB").text(chartsData.callTimeLengthB);
        $("#spanLineC").text(chartsData.callTimeLengthC);
        callCntDom.setOption(initOption("话务总量",["话务总量"],chartsData.xAxisData, callCntSeries));
        callTimeLengthDom.setOption(initOption("累计通话时长（秒）",["A路","B路","C路"],chartsData.xAxisData, thscsumSeries));
        successCntDom.setOption(initOption("接通率",["A路","B路","C路"],chartsData.xAxisData, succcntSeries));
        answerCntDom.setOption(initOption("应答率",["A路","B路","C路"],chartsData.xAxisData, answercntSeries));
    }
            ...
            if(data){
                ...
                for (j=0; j<data.length; j++) {
                    if(xAxisData[i] == data[j]['d']){
                        ...
                        if(data[j]['thscsumC']) chartsData.callTimeLengthC += parseInt(data[j]['thscsumC']);
    ...
    <div class="mt-20">
        ...
        <div style="position: relative; float: left;">
            <div style="position: absolute; margin-top: 30px; margin-left: 500px; font-size: 12px;">
                当前累计通话时长[A路:<span id="spanLineA">0</span>&nbsp;&nbsp;&nbsp;B路:<span id="spanLineB">0</span>&nbsp;&nbsp;&nbsp;C路:<span id="spanLineC">0</span>]
            </div>
