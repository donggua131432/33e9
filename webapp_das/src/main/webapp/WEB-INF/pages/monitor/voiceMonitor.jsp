...
<script type="text/javascript">
    ...
    function loadCharts(sType){
        var data = loadAjaxData('<c:url value="/monitor/voiceMonitorData"/>',{sType:sType,day:$("#historyDateTime").val()}),
            ...
            if(data){
                ...
                for (j=0; j<data.length; j++) {
                    if(xAxisData[i] == data[j]['d']){
                        ...
                        //接通率
                        chartsData.succcnt.push(data[j]['succcnt']!=undefined?data[j]['succcnt']:0);
                        //应答率
                        chartsData.answercnt.push(data[j]['answercnt']!=undefined?data[j]['answercnt']:0);
                        flag = true;
                    }
                }
                if(flag == false){
