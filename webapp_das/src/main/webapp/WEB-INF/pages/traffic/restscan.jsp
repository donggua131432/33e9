...
<script type="text/javascript">
    ...
    function initcharts(stype){
        var data = loadData('<c:url value="/traffic/restscan"/>',{stype:stype});
        var xyAxis = parseData(data,stype);

        // 使用刚指定的配置项和数据显示图表。
        thscTotal.setOption(initOption("累计通话时长(秒)", ["A路","B路"], xyAxis['d'], [xyAxis['thscsumA'],xyAxis['thscsumB']] ));
        thscAvg.setOption(initOption("平均通话时长(秒)", ["A路","B路"], xyAxis['d'], [xyAxis['thscAvgA'],xyAxis['thscAvgB']] ));
