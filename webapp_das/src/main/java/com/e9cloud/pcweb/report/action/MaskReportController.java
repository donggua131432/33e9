...
/**
 * Created by lixin on 2016/12/5.
 */

@Controller
@RequestMapping("/mask")
public class MaskReportController extends BaseController {
    @Autowired
    private MaskPhoneService maskPhoneService ;

    // 跳转到 专号通 的话务报表页面  maskReportlist
    @RequestMapping("/toMaskList")
    public String toMaskList() {
        return "report/maskReportList";
    }
     ...
     * @return pageWrapper
     */
    @RequestMapping("/pageMaskList")
    @ResponseBody
    public PageWrapper pageMaskList(Page page){
        logger.info("------------------- MaskReportController pageMaskList START -----------------------------");
        page.getParams().get("reportType");
        PageWrapper pageWrapper = maskPhoneService.pageMaskList(page);
        return pageWrapper;
    }
     ...
     * @return
     */
    @RequestMapping("/downloadMaskReport")
    public ModelAndView downloadMaskReport(Page page) {
        logger.info("=====================================MaskReportController downloadMaskReport Execute=====================================");
            List<Map<String, Object>> totals = maskPhoneService.downloadMaskReport(page);
            ...
            map.put("list", list);
            map.put("title", "专号通话务报表信息");
            map.put("excelName","专号通话务报表信息");
