...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		if (user != null) {
			if (Tools.isNullStr(user.getSysType()) || !user.getSysType().contains(Constants.RMS_USER_SYS_TYPE_D)) { // 用户系统权限
				throw new SystemPermissionException("no permission to login this system");
			}
			if(Constants.RMS_USER_STATUS_DISABLED.equals(user.getStatus())){ // 用户被禁用
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		//info.addRole(shiroUser.getUser());
		//info.addRoles();
		return info;
	}
